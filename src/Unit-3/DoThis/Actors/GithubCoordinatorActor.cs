...
namespace GithubActors.Actors
{
	...
	public class GithubCoordinatorActor : ReceiveActor
	{
		...
		{
			var workerProps = Props.Create(() => new GithubWorkerActor(GithubClientFactory.GetClient))
				.WithRouter(new RoundRobinPool(10));

			_githubWorker = Context.ActorOf(workerProps);
		}
		...
		private void Working()
		{
			...
			Receive<GithubWorkerActor.StarredReposForUser>(user =>
			{
				...
				{
					if (!_similarRepos.ContainsKey(repo.HtmlUrl)) _similarRepos[repo.HtmlUrl] = new SimilarRepo(repo);
			...
			Receive<PublishUpdate>(update =>
			{
				...
				if (_receivedInitialUsers && _githubProgressStats.IsFinished)
				{
					...
					foreach (var subscriber in _subscribers) subscriber.Tell(sortedSimilarRepos);
					BecomeWaiting();
				}

				foreach (var subscriber in _subscribers) subscriber.Tell(_githubProgressStats);
			});
			...
			Receive<GithubCommanderActor.CanAcceptJob>(job =>
				Sender.Tell(new GithubCommanderActor.UnableToAcceptJob(job.Repo)));
			...
			{
				_receivedInitialUsers = true;
				foreach (var subscriber in _subscribers) subscriber.Tell(new JobFailed(_currentRepo));
				BecomeWaiting();
			});

			//query failed, can't be retried, and it's a QueryStarrers operation - means individual operation failed
			Receive<RetryableQuery>(query => !query.CanRetry && query.Query is GithubWorkerActor.QueryStarrer,
